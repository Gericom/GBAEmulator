...
void DMA::RunCycle()
{
	...
	if (dma0 >> 15)
	{
		if (!mDMAChannels[0].busy)
		{
			...
			mDMAChannels[0].srcMode = (dma0 >> 5) & 0;
			mDMAChannels[0].dst = *((uint32_t*)&mIORegisters->mMemory[0xB4]);
			mDMAChannels[0].dstMode = (dma0 >> 7) & 0;
		...
		if (mDMAChannels[0].waitingForData && mMemoryBus->IsDataReady())
		{
			...
			if (!mDMAChannels[0].dstMode) mDMAChannels[0].dst += (mDMAChannels[0].bitMode ? 4 : 2);
		...
		if (mDMAChannels[0].nrLeft > 0 && !mMemoryBus->IsBusy())
		{
	...
	if (dma1 >> 15 && ((dma1 >> 12) & 3) == 0)
	{
		...
		if (mDMAChannels[1].waitingForData && mMemoryBus->IsDataReady())
		{
			...
			if (!mDMAChannels[1].dstMode) mDMAChannels[1].dst += (mDMAChannels[1].bitMode ? 4 : 2);
		...
		if (mDMAChannels[1].nrLeft > 0 && !mMemoryBus->IsBusy())
		{
	...
	if (dma2 >> 15 && ((dma2 >> 12) & 3) == 0)
	{
		...
		if (mDMAChannels[2].waitingForData && mMemoryBus->IsDataReady())
		{
			...
			if (!mDMAChannels[2].dstMode) mDMAChannels[2].dst += (mDMAChannels[2].bitMode ? 4 : 2);
	...
	if (dma3 >> 15 && ((dma3 >> 12) & 3) == 0)
	{
		...
		if (mDMAChannels[3].waitingForData && mMemoryBus->IsDataReady())
		{
			...
			if (!mDMAChannels[3].dstMode) mDMAChannels[3].dst += (mDMAChannels[3].bitMode ? 4 : 2);
		...
		if (mDMAChannels[3].nrLeft > 0 && !mMemoryBus->IsBusy())
		{
