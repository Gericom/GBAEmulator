	...
	if (!(mClockCounter % 4))//One dot takes 4 cycles
	{
		...
		*((uint16_t*)&mIORegisters->mMemory[6]) = mY;
		if (mX < 240 && mY < 160)//visible dots
		{
			...
			//bool bg0visible = false, bg1visible = false, bg2visible = false, bg3visible = false, oamvisible = false;
			for (int prio = 3; prio >= 0; prio--)
			{
				if ((*((uint16_t*)&mIORegisters->mMemory[0]) >> 11) & 1 && (*((uint16_t*)&mIORegisters->mMemory[0xE]) & 3) == prio)//bg3
				{
					if (bgmode == 0)
					{
						...
						int x1 = -(*((uint16_t*)&mIORegisters->mMemory[0x1C]) & 0x1FF);
						int y1 = -(*((uint16_t*)&mIORegisters->mMemory[0x1E]) & 0x1FF);
						int x2 = x1 + w;
						int y2 = y1 + h;
					...
					//{
						...
						if (x1 <= (mX << 8) && x2 > (mX << 8) && y1 <= (mY << 8) && y2 > (mY << 8))
						{
							...
							if (bgmode >= 1)
							{
								...
								if (dx < 0 || dx >= (w << 8) || dy < 0 || dy >= (h << 8)) continue;
							}
							dx >>= 8;
							dy >>= 8;
							int tilex = dx >> 3;
							...
							if (bgmode == 0)
							{
								text = *((uint16_t*)&mVRAM->mMemory[mapoffset + (tiley * (bgmode >= 1 ? w / 8 : 32) + tilex) * 2]);
							...
							{
								tilestart = tileoffset + mVRAM->mMemory[mapoffset + (tiley * (bgmode >= 1 ? w / 8 : 32) + tilex)] * tilesize;
								tilestart += tsy * (tilesize >> 3);
								tilestart += (tsx * tilesize) >> 6;
							}
							int color = 0;
				...
				if ((*((uint16_t*)&mIORegisters->mMemory[0]) >> 9) & 1 && (*((uint16_t*)&mIORegisters->mMemory[0xA]) & 3) == prio)//bg1
				{
					if (bgmode == 0 || bgmode == 1)
					{
						...
						int x1 = -(*((uint16_t*)&mIORegisters->mMemory[0x14]) & 0x1FF);
						int y1 = -(*((uint16_t*)&mIORegisters->mMemory[0x16]) & 0x1FF);
						int x2 = x1 + w;
						int y2 = y1 + h;
						if (x1 <= mX && x2 > mX && y1 <= mY && y2 > mY)
						{
				...
				if ((*((uint16_t*)&mIORegisters->mMemory[0]) >> 8) & 1 && (*((uint16_t*)&mIORegisters->mMemory[0x8]) & 3) == prio)//bg0
				{
					if (bgmode == 0 || bgmode == 1)
					{
						...
						int x1 = -(*((uint16_t*)&mIORegisters->mMemory[0x10]) & 0x1FF);
						int y1 = -(*((uint16_t*)&mIORegisters->mMemory[0x12]) & 0x1FF);
						int x2 = x1 + w;
						int y2 = y1 + h;
						if (x1 <= mX && x2 > mX && y1 <= mY && y2 > mY)
						{
						...
						if (pOAM->rsMode != 2 && pOAM->priority == prio)
						{
							...
							else*/
							{
								...
								if (x1 <= mX && x2 > mX && y1 <= mY && y2 > mY)
								{
									...
									{
										tilestart = oamtileoffset + pOAM->charNo * 32 + tiley * (w / 8) * 32 + tilex * 32;
									}
									int tsx = dx - (tilex << 3);
									int tsy = dy - (tiley << 3);
									tilestart += tsy * (tilesize >> 3);
			...
			//We need to find out the right order now
			//GXRgb pixel = backdrop;
			/*for (int i = 3; i >= 0; i--)//bg first
			{
				if ((*((uint16_t*)&mIORegisters->mMemory[0x8]) & 3) == i)
					pixel = (!bg0visible) ? pixel : bg0;
				else if ((*((uint16_t*)&mIORegisters->mMemory[0xA]) & 3) == i)
					pixel = (!bg1visible) ? pixel : bg1;
				else if ((*((uint16_t*)&mIORegisters->mMemory[0xC]) & 3) == i)
					pixel = (!bg2visible) ? pixel : bg2;
				else if ((*((uint16_t*)&mIORegisters->mMemory[0xE]) & 3) == i)
					pixel = (!bg3visible) ? pixel : bg3;
			}
			pixel = (!oamvisible) ? pixel : oam;*/
			//blit to screen
