...
void VRAM::HandleRequest(MemoryBus* memoryBus)
{
	...
	if (memoryBus->mRequestAddress >= 0x06000000 && memoryBus->mRequestAddress < 0x06018000)
	{
		...
		case MEMORYBUS_REQUEST_READ_16:
			memoryBus->mRequestData = mMemory[memoryBus->mRequestAddress - 0x06000000] | (mMemory[(memoryBus->mRequestAddress - 0x06000000) + 1] << 8);
			memoryBus->mRequestComplete = true;
			break;
		case MEMORYBUS_REQUEST_READ_32:
			memoryBus->mRequestData = mMemory[memoryBus->mRequestAddress - 0x06000000] | (mMemory[(memoryBus->mRequestAddress - 0x06000000) + 1] << 8) |
				(mMemory[(memoryBus->mRequestAddress - 0x06000000) + 2] << 16) | (mMemory[(memoryBus->mRequestAddress - 0x06000000) + 3] << 24);
			memoryBus->mRequestComplete = true;
			break;
		...
		case MEMORYBUS_REQUEST_WRITE_16:
			mMemory[memoryBus->mRequestAddress - 0x06000000] = memoryBus->mRequestData & 0xFF;
			mMemory[(memoryBus->mRequestAddress - 0x06000000) + 1] = (memoryBus->mRequestData >> 8) & 0xFF;
			memoryBus->mRequestComplete = true;
			break;
		case MEMORYBUS_REQUEST_WRITE_32:
			mMemory[memoryBus->mRequestAddress - 0x06000000] = memoryBus->mRequestData & 0xFF;
			mMemory[(memoryBus->mRequestAddress - 0x06000000) + 1] = (memoryBus->mRequestData >> 8) & 0xFF;
			mMemory[(memoryBus->mRequestAddress - 0x06000000) + 2] = (memoryBus->mRequestData >> 16) & 0xFF;
			mMemory[(memoryBus->mRequestAddress - 0x06000000) + 3] = (memoryBus->mRequestData >> 24) & 0xFF;
			memoryBus->mRequestComplete = true;
			break;
		default:
			break;
