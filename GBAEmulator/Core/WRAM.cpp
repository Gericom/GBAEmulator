...
void WRAM::HandleRequest(MemoryBus* memoryBus)
{
	...
	if (memoryBus->mRequestAddress >= 0x03000000 && memoryBus->mRequestAddress < 0x03008000)
	{
		...
		case MEMORYBUS_REQUEST_READ_8:
			memoryBus->mRequestData = mMemory[memoryBus->mRequestAddress - 0x03000000];
		...
		case MEMORYBUS_REQUEST_WRITE_8:
			mMemory[memoryBus->mRequestAddress - 0x03000000] = memoryBus->mRequestData & 0xFF;
			memoryBus->mRequestComplete = true;
			break;
		case MEMORYBUS_REQUEST_WRITE_16:
			mMemory[memoryBus->mRequestAddress - 0x03000000] = memoryBus->mRequestData & 0xFF;
			mMemory[memoryBus->mRequestAddress - 0x03000000 + 1] = (memoryBus->mRequestData >> 8) & 0xFF;
			memoryBus->mRequestComplete = true;
			break;
		case MEMORYBUS_REQUEST_WRITE_32:
			mMemory[memoryBus->mRequestAddress - 0x03000000] = memoryBus->mRequestData & 0xFF;
			mMemory[memoryBus->mRequestAddress - 0x03000000 + 1] = (memoryBus->mRequestData >> 8) & 0xFF;
			mMemory[memoryBus->mRequestAddress - 0x03000000 + 2] = (memoryBus->mRequestData >> 16) & 0xFF;
			mMemory[memoryBus->mRequestAddress - 0x03000000 + 3] = (memoryBus->mRequestData >> 24) & 0xFF;
			memoryBus->mRequestComplete = true;
			break;
		default:
			break;
